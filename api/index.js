...
// Subscriber Management Added (INSERT Only Logic)
// ============================================
...
// ============================================
// NEWSLETTER SUBSCRIBER API (INSERT ONLY - Toggle Logic)
// ============================================

// POST - Subscribe/Unsubscribe Toggle (public endpoint)
app.post('/api/subscribe', async (req, res) => {
    try {
        ...
        // If subscriber exists and is active -> UNSUBSCRIBE
        if (existingSubscriber && existingSubscriber.is_active) {
            // Delete old record and insert new inactive record
            ...
            const { data: unsubscribed, error: insertError } = await supabase
                ...
                .insert([{
                    ...
                    is_active: false,
                    subscribed_at: existingSubscriber.subscribed_at || new Date().toISOString(),
        ...
        // If subscriber exists but inactive -> RE-SUBSCRIBE
        if (existingSubscriber && !existingSubscriber.is_active) {
            // Delete old record and insert new active record
            const { error: deleteError } = await supabase
                .from('subscribers')
                .delete()
                .eq('id', existingSubscriber.id);
