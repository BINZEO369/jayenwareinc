(function() {
    ...
    function handleImageError(img) {
        if (img && img.classList) {
            ...
            var wrapper = img.closest('.new-arrival-card-image-wrapper');
    ...
    function setupCard(card, product, colors, slug) {
        var slider = card.querySelector('.new-arrival-image-slider');
        var dots = card.querySelector('.new-arrival-image-dots');
        var colorDots = card.querySelector('.new-arrival-color-dots-below');
    ...
    async function fetchNewArrivals() {
        try {
            var data = await fetchWithRetry(API_CONFIG.baseURL + API_CONFIG.endpoints.newArrivals);
            if (data.length) return data.filter(function(p) { return p.is_new_arrival === true || p.is_new_arrival === 1 || p.is_new_arrival === 'true'; });
            var all = await fetchWithRetry(API_CONFIG.baseURL + API_CONFIG.endpoints.products);
            if (all.length) {
                return all.filter(function(p) { return p.is_new_arrival === true || p.is_new_arrival === 1 || p.is_new_arrival === 'true'; })
    ...
    function initLazyLoading() {
        ...
        document.querySelectorAll('.new-arrival-card-image').forEach(function(img) { obs.observe(img); });
    }
    ...
    async function renderNewArrivals(products) {
        ...
        try {
            ...
            if (Array.isArray(products) && products.length > 0) {
                arrivals = products.filter(function(p) { return p.is_new_arrival === true || p.is_new_arrival === 1 || p.is_new_arrival === 'true'; });
            } else {
            ...
            if (arrivals && arrivals.length) {
                arrivals.slice(0, CONFIG.maxProducts).forEach(function(p) { grid.appendChild(createProductCard(p)); });
            } else {
                sec.classList.add('new-arrival-empty-section');
    ...
    function injectStyles() {
        ...
        style.textContent = '.new-arrivals-grid-section { padding: 32px 0; max-width: 100%; margin: 0 auto; background: #fff; }' +
            ...
            '.new-arrival-image-dots { position: absolute; bottom: 10px; left: 50%; transform: translateX(-50%); display: flex; gap: 6px; z-index: 4; pointer-events: auto; padding: 4px 8px; background: transparent; }' +
            '.new-arrival-image-dot { width: 6px; height: 6px; border-radius: 50%; background: rgba(255,255,255,0.55); cursor: pointer; transition: all 0.35s cubic-bezier(0.4, 0, 0.2, 1); flex-shrink: 0; box-shadow: 0 1px 2px rgba(0,0,0,0.15); backdrop-filter: blur(4px); -webkit-backdrop-filter: blur(4px); }' +
            '.new-arrival-image-dot.active { background: #ffffff; width: 8px; height: 8px; box-shadow: 0 1px 4px rgba(0,0,0,0.25); }' +
            '.new-arrival-image-dot:hover { background: rgba(255,255,255,0.85); transform: scale(1.2); }' +
    ...
    function init() {
        ...
        window.addEventListener('jayenware:dataLoaded', function(e) {
            ...
            if (detail.products && Array.isArray(detail.products)) {
                var arr = detail.products.filter(function(p) { return p.is_new_arrival === true || p.is_new_arrival === 1; });
        ...
        if (window.currentData && window.currentData.products && Array.isArray(window.currentData.products)) {
            var arr = window.currentData.products.filter(function(p) { return p.is_new_arrival === true || p.is_new_arrival === 1; });
            if (arr.length) setTimeout(function() { renderNewArrivals(arr); }, 50);
        }
    }
